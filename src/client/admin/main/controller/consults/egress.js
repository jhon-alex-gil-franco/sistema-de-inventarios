...
export  const consultAllEgress=(doc)=>{
    consultData(`${url}egress`).then(resp => {
...
export const consultEgress=(doc)=>{
    ...
    const consultOnEvents=()=>{
        dropChildOfTable (doc.containerTableEgress) 
        let dir=""
        if(validateDate(doc.date1.value)&&doc.date2.value==""){dir=`${url}egress/`+doc.date1.value +"/"+doc.date1.value
         }if(validateDate(doc.date2.value)&&validateDate(doc.date1.value)){dir=`${url}egress/`+doc.date1.value +"/"+doc.date2.value
           }if(doc.date1.value==="")dir=`${url}egress` 
