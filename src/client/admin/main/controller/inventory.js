                        ...
                        case "name":
                            ...
                            case "ref":
                                ...
                                case "mark":
                                    ...
                                    case "cat":
                                        const listCat = []
                                        const foundCat = resp.map(data => { if (data.Categoria.includes(doc.inputSeach.value.toUpperCase())) { listCat.push(createDataForTable(data)) } })
                                        listCat.length > 0 ? createTableConsult(doc, listCat) : mesajeNoFundData(doc.containerTableInventory);
                                        break
                }
           });
...
const createTableConsult = (doc,  res) => {
    ...
    th9.textContent = "Categoria"
    th10.textContent = "Entradas"
    tr.appendChild(th1);
    tr.appendChild(th2);
    tr.appendChild(th3);
    tr.appendChild(th4);
    ...
    for (const list of res) {
        ...
        td9.textContent = `${list.Categoria}`;
        btnIn.className="btn btn-info"
        const i=document.createElement("i");
        i.className="fas fa-sign-in-alt"
        btnIn.className = "btn btn-info";
        btnIn.appendChild(i)
        td10.appendChild(btnIn);
        trd.appendChild(td1);
        trd.appendChild(td2);
        trd.appendChild(td3);
        trd.appendChild(td4);
    ...
    strong.textContent="sistema de inventario"
