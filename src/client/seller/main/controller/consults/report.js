...
export const reportsEmployee=(doc)=>{            
    if(validateDate(doc.dateEmp1.value)&&doc.dateEmp2.value==""){
        url="http://localhost:4000/api/servicesByEmploye/"+doc.dateEmp1.value +"/"+doc.dateEmp1.value+"/"+doc.selectListEmployee.value
    }if(validateDate(doc.dateEmp2.value)&&validateDate(doc.dateEmp1.value)){
        url="http://localhost:4000/api/servicesByEmploye/"+doc.dateEmp1.value +"/"+doc.dateEmp2.value+"/"+doc.selectListEmployee.value
      
    }
    consultData(url).then(resp=>{
        // console.log(resp)
        if(resp=="Void"|| resp==undefined){
            swal("No se encontraron resultados. Por favor verifique los parametros ingresados!", {
                icon: "info",
              });
        }else{
